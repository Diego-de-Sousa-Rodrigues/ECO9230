...
if sum(Tran') ~= ones(1,N)
   ...
   disp(['rows ',num2str(str),' do not add up to one']);
end
...
test = 1;
  ...
  while test > 10^(-8);
   ...
   disp(sprintf('lambda of original process vs. that of Markov chain'));
   disp(sprintf('Original process: %d; Markov chain: %d',lambda,alambda));
   
   
   disp(sprintf('Standard deviation of true process vs. that of Markov chain'));
   disp(sprintf('Original process: %d; Markov chain: %d',stvy,asigmay));
   disp(' ');
